...
import {
  AlertTriangle, CalendarClock, MessageSquare,
  Radio, RefreshCw, Scan, ShieldAlert,
  ThumbsDown, ThumbsUp,
} from "lucide-react";
...
function KpiCard({ icon, label, value, sub, accent = "default" }: {
  ...
  sub?: string; accent?: "default" | "blue" | "green" | "red" | "yellow" | "purple";
}) {
  const colors = {
    default: "bg-zinc-800/60 text-zinc-400", blue:   "bg-blue-500/10 text-blue-400",
    green:   "bg-green-500/10 text-green-400", red:  "bg-red-500/10 text-red-400",
    yellow:  "bg-yellow-500/10 text-yellow-400", purple: "bg-purple-500/10 text-purple-400",
  };
  return (
...
export default function SocialOverviewPage() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const data = await fetchMentionStats();
      setStats(data);
    } catch (e) {
  ...
  const handleScan = async () => {
    setScanning(true);
    try { await triggerScan(); await load(true); }
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-zinc-500 mt-1 text-sm">Real-time brand mentions, risk scoring, and sentiment analysis.</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Button size="sm" onClick={handleScan} disabled={scanning || loading}
      ...
      {error && (
        <div className="rounded-lg border border-red-900/50 bg-red-950/30 px-4 py-3 text-sm text-red-400 font-mono">{error}</div>
      )}

      {/* KPI Grid */}
      {loading ? (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          {Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-32 rounded-xl bg-zinc-800/50" />)}
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <KpiCard icon={<ThumbsUp       className="h-4 w-4" />} label="Positive"       value={positiveCount.toLocaleString()}  sub={`${positivePct}%`}      accent="green"  />
          <KpiCard icon={<ThumbsDown     className="h-4 w-4" />} label="Negative"       value={negativeCount.toLocaleString()}  sub={`${negativePct}%`}      accent="red"    />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-6 bg-zinc-900/50 border-zinc-800">
          ...
          {loading ? <Skeleton className="h-56 w-full rounded-lg bg-zinc-800/50" /> :
            chartData.some(d => d.count > 0) ? (
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: "#09090b", border: "1px solid #27272a", borderRadius: 8, fontSize: 12 }} formatter={(v: number) => [v, "Mentions"]} />
                  <Area type="monotone" dataKey="count" stroke="#3b82f6" fill="url(#mg)" strokeWidth={2} name="Mentions" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-56 flex items-center justify-center text-sm text-zinc-600">No mention data yet — add keywords and run a scan.</div>
            )
          }
        </Card>
        ...
        <Card className="p-6 bg-zinc-900/50 border-zinc-800">
          ...
          {loading ? <Skeleton className="h-48 w-full rounded-full bg-zinc-800/50" /> :
            pieData.length > 0 ? (
              <>
          ...
          <div className="rounded-xl border border-zinc-800 overflow-hidden">
            <table className="w-full text-left">
              <thead className="border-b border-zinc-800 bg-zinc-900/80">
                <tr>{["Status", "Mentions Found", "High Risk", "Scanned At"].map(h =>
                ...
                {stats.recentScans.map(scan => (
                  <tr key={scan.id} className="hover:bg-zinc-800/20">
                    <td className="px-5 py-3">
                      <Badge variant="outline" className={scan.scanStatus === "SUCCESS" ? "border-green-500/30 text-green-400 bg-green-500/5" : "border-red-500/30 text-red-400 bg-red-500/5"}>
                        {scan.scanStatus}
                      </Badge>
                    </td>
